...
const TaskList = () => {
  ...
  const [activeTab, setActiveTab] = useState<FilterOption>(location.state?.activeTab || 'all');
  const [isModalOpen, setIsModalOpen] = useState(false);

  const { tasks, setRefreshTrigger, editTaskId, setEditTaskId, loadMoreTasks, hasMore, isLoadingMore, total, allCount, countsByStatus, deleteTask } = useTask(activeTab === 'all' ? undefined : activeTab);
  ...
  const tabs: Tab[] = [
    { key: 'all', label: 'All Tasks', statusColor: getStatusColor('all'), count: () => allCount ?? total },
    { key: STATUS.TODO, label: 'To Do', statusColor: getStatusColor(STATUS.TODO), count: () => countsByStatus?.todo ?? 0 },
    { key: STATUS.IN_PROGRESS, label: 'In Progress', statusColor: getStatusColor(STATUS.IN_PROGRESS), count: () => countsByStatus?.in_progress ?? 0 },
    { key: STATUS.COMPLETED, label: 'Completed', statusColor: getStatusColor(STATUS.COMPLETED), count: () => countsByStatus?.completed ?? 0 },
  ];
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="p-4 space-y-8 border-t border-gray-200 mt-7 dark:border-gray-800 sm:mt-0 xl:p-6">
          {(activeTab === 'all' || activeTab === STATUS.TODO) && (
            <TaskGroup title="To Do" tasks={todoTasks} setShowModal={setIsModalOpen} setEditTaskId={setEditTaskId} deleteTask={deleteTask} activeTab={activeTab} setActiveTab={setActiveTab} onLoadMore={loadMoreTasks} hasMore={hasMore} isLoadingMore={isLoadingMore} totalCount={countsByStatus?.todo} />
          )}
          {(activeTab === 'all' || activeTab === STATUS.IN_PROGRESS) && (
            <TaskGroup title="In Progress" tasks={inprogressTasks} setShowModal={setIsModalOpen} setEditTaskId={setEditTaskId} deleteTask={deleteTask} activeTab={activeTab} setActiveTab={setActiveTab} onLoadMore={loadMoreTasks} hasMore={hasMore} isLoadingMore={isLoadingMore} totalCount={countsByStatus?.in_progress} />
          )}
          {(activeTab === 'all' || activeTab === STATUS.COMPLETED) && (
            <TaskGroup title="Completed" tasks={completedTasks} setShowModal={setIsModalOpen} setEditTaskId={setEditTaskId} deleteTask={deleteTask} activeTab={activeTab} setActiveTab={setActiveTab} onLoadMore={loadMoreTasks} hasMore={hasMore} isLoadingMore={isLoadingMore} totalCount={countsByStatus?.completed} />
          )}
        </div>
      </div>
    </>
